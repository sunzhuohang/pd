...
type ElasticSchedulerManager interface {
	Sync(tac *v1alpha1.TidbClusterAutoScaler) error
}

type elasticSchedulerManager struct {
...
func (esm *elasticSchedulerManager) Sync(tac *v1alpha1.TidbClusterAutoScaler) error {
	...
	tc, err := esm.cli.PingcapV1alpha1().TidbClusters(tac.Spec.Cluster.Namespace).Get(tcName, metav1.GetOptions{})
	if err != nil {
	...
	oldTc := tc.DeepCopy()
	if err := esm.syncAutoScaling(tc, tac); err != nil {
		return err
	}
	if err := esm.syncTidbClusterReplicas(tc, oldTc); err != nil {
		return err
	}
	return esm.syncAutoScalingStatus(tc, oldTc, tac)
}

func (esm *elasticSchedulerManager) syncAutoScaling(tc *v1alpha1.TidbCluster, tac *v1alpha1.TidbClusterAutoScaler) error {
	defaultTAC(tac)
	oldTikvReplicas := tc.Spec.TiKV.Replicas
	if err := esm.syncTiKV(tc, tac); err != nil {
	...
	if err := esm.syncTiDB(tc, tac); err != nil {
...
func (esm *elasticSchedulerManager) syncTidbClusterReplicas(tc *v1alpha1.TidbCluster, oldTc *v1alpha1.TidbCluster) error {
	if tc.Spec.TiDB.Replicas == oldTc.Spec.TiDB.Replicas && tc.Spec.TiKV.Replicas == oldTc.Spec.TiKV.Replicas {
		return nil
	}
	newTc := tc.DeepCopy()
	_, err :=esm.tcControl.UpdateTidbCluster(newTc, &newTc.Status, &oldTc.Status)
	if err != nil {
		return err
	}
	return nil
...
func (esm *elasticSchedulerManager) syncAutoScalingStatus(tc *v1alpha1.TidbCluster, oldTc *v1alpha1.TidbCluster,
	tac *v1alpha1.TidbClusterAutoScaler) error {
	return esm.updateTidbClusterAutoScaler(tac)
}

func (esm *elasticSchedulerManager) updateTidbClusterAutoScaler(tac *v1alpha1.TidbClusterAutoScaler) error {
	...
	return retry.RetryOnConflict(retry.DefaultRetry, func() error {
		var updateErr error
		_, updateErr = esm.cli.PingcapV1alpha1().TidbClusterAutoScalers(ns).Update(tac)
		if updateErr == nil {
		...
		if updated, err := esm.taLister.TidbClusterAutoScalers(ns).Get(tacName); err == nil {
